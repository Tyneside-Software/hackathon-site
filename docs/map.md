...
**Show buses** / **Hide buses**. Off means no request at all. While on: 30-mile **square** bbox fetch, then haversine-filter to a 30-mile **circle** of Newcastle. Line-number `divIcon`s, operator colour when the feed sends a hex. Hide aborts the in-flight fetch, stops the timer, removes markers. Hidden tab: skip the tick.
